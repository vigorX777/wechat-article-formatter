...
> 基于参考文章 https://mp.weixin.qq.com/s/5OqgvE1pz4fKtS3lHp0xoA 提炼，已在公众号编辑器中验证通过。
...
- 前言卡片使用普通段落，不做引用块视觉。
- `WECHATIMGPH_1` 放入前言卡片内部，位于前言文字之后。
...
| 变量名 | 默认值 | 派生规则 | 用途 |
|:---|:---|:---|:---|
| `{主色}` | `#fb923c` | 用户提供 | 渐变起始、头部背景 |
| `{主色-深}` | `#ea580c` | 主色 L-15% | 渐变结束 |
| `{强调色}` | `#d97706` | 主色 L-10%, S+5% | 边框、序号方块、高亮文字 |
| `{深色文字}` | `#78350f` | 主色 L=20% | 引用块文字 |
| `{背景-亮}` | `#fff9e6` | 主色 S=20%, L=95% | 引用块渐变起点 |
| `{背景-深}` | `#fef3c7` | 主色 S=30%, L=90% | 引用块渐变终点 |

**渐变替换规则：**
- 头部渐变：`linear-gradient(135deg, {主色} 0%, {主色-深} 100%)`
- 序号方块：`linear-gradient(135deg, {强调色} 0%, {主色-深} 100%)`
- 引用块背景：`linear-gradient(135deg, {背景-亮} 0%, {背景-深} 100%)`
- 分隔线：`linear-gradient(90deg, transparent, rgba({强调色RGB}, 0.6), transparent)`
...
**用途：** 文章分类标签，如「行业快评」「深度解读」「技术分享」「年度总结」。应使用源 Markdown 明确提供的分类字段，不要临时猜测。
...
<tbody><tr>
<td style="background-color: #fff9e6; border: 1px solid #d97706; color: #d97706; padding: 8px; text-align: center; font-weight: bold; font-size: 14px; border-radius: 8px;">
WECHATIMGPH_1
</td>
</tr></tbody>
</table>
