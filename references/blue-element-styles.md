...
| 变量名 | 默认值 | 派生规则 | 用途 |
| :--- | :--- | :--- | :--- |
| `{主色}` | `#2563eb` | 用户提供 | 标题边框、强调文字、序号方块 |
| `{主色-深}` | `#1d4ed8` | 主色 L-10% | 渐变结束 |
| `{强调色}` | `#3b82f6` | 主色 L+10% | 渐变起始、按钮 |
| `{背景-浅}` | `#eff6ff` | 主色 S=20%, L=97% | 引用块背景渐变起点 |
| `{背景-深}` | `#dbeafe` | 主色 S=30%, L=92% | 引用块背景渐变终点 |
| `{深色文字}` | `#1e3a5f` | 主色 L=25% | 引用块内文字 |

**渐变替换规则：**
- 头部渐变：`linear-gradient(135deg, {强调色} 0%, {主色-深} 100%)`
- 序号方块：`linear-gradient(135deg, {主色} 0%, {主色-深} 100%)`
- 引用块背景：`linear-gradient(135deg, {背景-浅} 0%, {背景-深} 100%)`
