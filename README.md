...
**特点**：H2 标题带左边框和底部虚线、引用块浅灰背景、更精致的排版间距、整体简洁清爽
...
- `manifest.json`：包含标题、图片映射等元信息

## 已知限制

### 长文档内容遗漏

由于 Markdown → HTML 转换依赖 LLM 逐行处理，**超过 300 行的文档可能出现内容遗漏**。

| 遗漏类型 | 典型表现 |
|----------|----------|
| 表格行丢失 | 原文 20 行表格，转换后只有 7 行 |
| 章节缺失 | 末尾章节（如附录、参考资料）完全消失 |
| 列表截断 | 长列表只保留前几项 |
...
| 要求 | 说明 |
|------|------|
| 容器 ID | 内容必须包含在 `id="output"` 的元素中 |
| 样式内联 | 所有 CSS 必须写在 `style` 属性中 |
...
发布脚本使用 `osascript` 发送键盘事件，需要辅助功能权限：
...
| 问题 | 解决方案 |
|------|----------|
| Chrome 未找到 | 安装 Chrome 或设置 `CHROME_PATH` 环境变量 |
| 粘贴内容为空 | 确保 HTML 包含 `id="output"` 容器 |
| 图片替换失败 | 检查 manifest.json 中的路径是否正确 |
| 内容不完整 | 长文档（>300行）需手动核对补充 |
...
- 🎨 **排版细节优化**：
  - 正文行高调整为 `1.75`，字间距 `0.05em`，提升阅读体验
  - H2 标题新增底部虚线装饰，增强视觉层级
  - 分隔线改为渐变样式，过渡更柔和
- 📦 **容器化设计**：
  - 内容区域增加极淡背景 (`rgba(0,0,0,0.02)`) 和圆角包裹
  - 增强内容与背景的层次感，更接近卡片式阅读体验

### v1.3.1 (2025-02-06)

**内容完整性检查 + 故障排查优化**

- 📝 **新增「已知限制」章节**：明确长文档（>300行）可能出现内容遗漏的问题
  - 表格行丢失、章节缺失、列表截断等典型表现
  - 提供转换后核对清单
- 📝 **新增「故障排查」章节**：
  - macOS 辅助功能权限配置步骤
  - 粘贴失败、内容为空等常见问题解决方案
- 🔧 **HTML 容器 ID 规范化**：统一使用 `id="output"` 作为内容容器（向后兼容 `#wechat-content`）
- 🔧 **优化 publish.ts**：调整选择器优先级，优先查找 `#output`
- 📝 **更新 SKILL.md 验证清单**：新增内容完整性检查项
